...
void AApexCharacterBase::Tick(float DeltaTime)
{
	...
	// === 신규: 궤적 미리보기 갱신 (로컬 컨트롤 only) ===
	if (bIsAimingThrowable && PendingThrowable && IsLocallyControlled())
	{
		FVector CamLoc;
		FRotator CamRot;
		GetActorEyesViewPoint(CamLoc, CamRot);
		PendingThrowable->UpdateAimPreview(CamLoc, CamRot);
	}
}
...
void AApexCharacterBase::GetLifetimeReplicatedProps(TArray<FLifetimeProperty>& OutLifetimeProps) const
{
	...
	DOREPLIFETIME_CONDITION(AApexCharacterBase, PendingThrowable, COND_OwnerOnly);
}
...
void AApexCharacterBase::OnRep_PendingThrowable()
{
	if (bIsAimingThrowable && PendingThrowable && IsLocallyControlled())
	{
		PendingThrowable->ShowAimPreview();
	}
}
...
void AApexCharacterBase::StartThrowableAim()
{
	...
	// === 신규: 궤적 미리보기 시작 (로컬 컨트롤 only) ===
	// PendingThrowable 은 Server_StartThrowableAim 안에서 스폰. 리슨서버 호스트는 즉시,
	// 원격 클라는 OnRep 도착 후 Tick 에서 UpdateAimPreview 가 작동.
	if (PendingThrowable && IsLocallyControlled())
	{
		PendingThrowable->ShowAimPreview();
	}
}
...
void AApexCharacterBase::StopThrowableAim()
{
	...
	if (PendingThrowable && IsLocallyControlled())
	{
		PendingThrowable->HideAimPreview();
	}
